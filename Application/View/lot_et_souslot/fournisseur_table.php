...
$query = "
    ...
    ORDER BY l.lot_name
";
$result_fournisseur = mysqli_query($conn, $query);
...
<table id="fournisseursTable" class="table-bordered table table-striped table-hover">
    ...
    while ($row = mysqli_fetch_assoc($result_fournisseur)) {
        echo '<tr>';
        ...
        echo '<a href="#" style="color:green;" class="modify-btn modifierFournisseurBtn" data-id="' . htmlspecialchars($row['id_fournisseur']) . '" data-lot-id="' . htmlspecialchars($row['lot_id']) . '">';
